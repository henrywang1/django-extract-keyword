...
class BeamSearchDecoder(object):
  ...
  def decode(self):
    ...
    while True:
      ...
      if FLAGS.single_pass:
        yield self.write_for_rouge(decoded_words, counter) # write ref summary and decoded summary to file, to eval with pyrouge later
        counter += 1 # this is how many examples we've decoded
      else:
        print_results(article_withunks, decoded_output) # log output to screen
        yield self.write_for_attnvis(article_withunks, decoded_words, best_hyp.attn_dists, best_hyp.p_gens) # write info to .json file for visualization tool
  ...
  def write_for_rouge(self, decoded_words, ex_index):
  ...
  def write_for_attnvis(self,article, decoded_words, attn_dists, p_gens):
    ...
    output_fname = os.path.join(self._decode_dir, 'att_vis_result.json')
    with open(output_fname, 'w') as output_file:
      json.dump(to_write, output_file)
    tf.logging.info('Wrote visualization data to %s', output_fname)
    return to_write['decoded_lst']

def print_results(article, decoded_output):
